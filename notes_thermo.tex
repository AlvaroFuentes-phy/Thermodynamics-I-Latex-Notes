...
Un sistema termodinámico es cualquier sistema material, el cual está sujeto a una serie de variables, como son la masa y el volumen, como mínimo. 
\\

Para determinar el estado de un sistema, se deben caracterizar sus variables. Si sus magnitudes son diferentes, lo será su estado. Además, un sistema material termodinámico interactúa con otros, lo que genera otra de las variables mínimas: la presión. 
\\

\begin{definicion}[Variables termodinámicas]
Caracterizan el sistema y determinan su estado; como mínimo, las que nos encontramos son $n,P,V,T$. Están relacionadas según
\begin{equation}
    F(V,n,T,P)=0
\end{equation}
...
Relacionan el resto de variables con las de estado, y no tienen por qué existir necesariamente. Por ejemplo:
\begin{equation*}
    PV=nRT
\end{equation*}
...
Para establecer si una variable es extensiva o intensiva, establecemos la siguiente regla:
\begin{itemize}
    \item Si depende del tamaño o de la cantidad de materia del sistema, será extensiva.
...
Está formado por todos los sistemas; está exento de interacciones termodinámicas.
\end{definicion}

\section{Equilibrio en un sistema}
Normalmente, para los sistemas, pongamos como ejemplo un recinto que contiene un fluido turbulento: las variables $A=A(\vec{r}, t)$, o $A=A(\vec{r})$ es estacionario pues no depende de $t$, o bien $A=A(t)$ es uniforme pues no depende de $\vec{r}$. Solo en el caso de que todas las variables intensivas del sistema sean estacionarias y uniformes podremos decir que el sistema está en equilibrio. Por lo que diremos que existe ecuación de estado. 

\section{Concepto de proceso termodinámico}
Se define un proceso termodinámico como el intercambio de energía con otros sistemas, diferenciando entre:
\begin{itemize}
    \item Procesos de relajación: de un estado inicial de equilibrio a uno final sin ningún intermedio. Además, duran un tiempo limitado y son irreversibles.
    \item Procesos cuasiestáticos: salta entre estados de equilibrio, desde el inicial hasta el final. Para que un proceso sea reversible será cuasiestático necesariamente; la implicación contraria es falsa.
...
Iniciaremos este nuevo tema estudiando el concepto de interacción termodinámica.
\begin{definicion}[Interacción Termodinámica]
...
\end{equation}
\section{Entropía para un gas ideal}
De cara a afrontar la parte práctica de la asignatura se presentan las siguientes expresiones que nos permiten calcular la variación de entropía para un gas ideal en función de las coordenadas dadas. Se trata de obtener las expresiones para la variaicón de entropía de un gas ideal que realiza un proceso cualquiera entre un estado inicial $(p_i, V_i, T_i)$ y uno final $(p_f,V_f, T_f)$
Si partimos de: 
\begin{equation}
    dU=\delta Q-pdV=TdS-pdV \rightarrow dS=\frac{1}{T}dU+\frac{p}{T}dV 
\end{equation}
y escribiendo $dU$ como
\begin{equation}
    dU=nC_VdT
\end{equation}
Podemos escribir la entropía de un gas ideal.
% --- Sección T, V ---
\textbf{En función de las variables $T, V$:}

\begin{equation}
    dS = \frac{1}{T}dU + \frac{p}{T}dV = nc_v \frac{dT}{T} + nR \frac{dV}{V}
\end{equation}
...
% --- Sección T, p ---
\textbf{En función de las variables $T, p$:}

\begin{equation}
    dV = \left( \frac{\partial V}{\partial T} \right)_p dT + \left( \frac{\partial V}{\partial p} \right)_T dp = \frac{nR}{p}dT - \frac{nRT}{p^2}dp
\end{equation}

\begin{equation}
    dS = nc_v \frac{dT}{T} + \frac{nR}{V} \left[ \frac{nR}{p}dT - \frac{nRT}{p^2}dp \right] = nc_v \frac{dT}{T} + nR \frac{dT}{T} - nR \frac{dp}{p} = nc_p \frac{dT}{T} - nR \frac{dp}{p}
\end{equation}
...
% --- Sección V, p ---
\textbf{En función de las variables $V, p$:}

\begin{equation}
    dT = \left( \frac{\partial T}{\partial p} \right)_V dp + \left( \frac{\partial T}{\partial V} \right)_p dV = \frac{V}{nR}dp + \frac{p}{nR}dV
\end{equation}

\begin{equation}
    dS = \frac{nc_v}{T} \left[ \frac{V}{nR}dp + \frac{p}{nR}dV \right] + \frac{nR}{V}dV = nc_v \frac{dp}{p} + nc_v \frac{dV}{V} + nR \frac{dV}{V} = nc_v \frac{dp}{p} + nc_p \frac{dV}{V}
\end{equation}

\begin{formula}
\begin{equation}
    \Delta S = nc_v \ln \frac{p_f}{p_i} + nc_p \ln \frac{V_f}{V_i}
\end{equation}
\end{formula}
...
\bibitem{callen}
H.~B.~Callen, \emph{Thermodynamics and an Introduction to Thermostatistics}, 2nd ed., Wiley, 1985.

\bibitem{zemansky}
M.~W.~Zemansky y R.~H.~Dittman, \emph{Heat and Thermodynamics}, 7th ed., McGraw--Hill, 1997.

\bibitem{atkins}
P.~Atkins y J.~de Paula, \emph{Physical Chemistry}, Oxford University Press (varias ediciones).

\bibitem{reif}
F.~Reif, \emph{Fundamentals of Statistical and Thermal Physics}, McGraw--Hill, 1965.

\bibitem{moran}
M.~J.~Moran, H.~N.~Shapiro, D.~D.~Boettner y M.~B.~Bailey, \emph{Fundamentals of Engineering Thermodynamics}, Wiley (varias ediciones).

\bibitem{feynman}
R.~P.~Feynman, R.~B.~Leighton y M.~Sands, \emph{The Feynman Lectures on Physics}, Vol.~I, Addison--Wesley, 1963 (cap\'itulos sobre termodin\'amica).

\bibitem{landau}
L.~D.~Landau y E.~M.~Lifshitz, \emph{Statistical Physics}, Part~1, Pergamon/Butterworth--Heinemann (varias ediciones).
